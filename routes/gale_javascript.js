...
function CalculateRisk(riskindex, CurrentAge, ProjectionAge, AgeIndicator, NumberOfBiopsy, MenarcheAge, FirstLiveBirthAge, EverHadBiopsy, FirstDegRelatives, ihyp, rhyp, irace)
{
	//based on current practicing guidelines and weights


	//covariates in Gail model (independent variable for the patient)
	var bet2 = new Array(8);
	for (var i = 0; i<8; i++){
		bet2[i] = new Array(12);
	}
	var bet = [8];
	...
	var rmu2 = new Array(14);
	for (var i = 0; i<14; i++){
		rmu2[i] = new Array(12);
	}
	var rlan2 = new Array(14);
	for (var i = 0; i<14; i++){
		rlan2[i] = new Array(12);
	}
	var rf2 = new Array(2);
	for (var i = 0; i<2; i++){
		rmu[i] = new Array(13);
...
//American-Asian conversion factor
    ...
    var ts, abss = 0.0;
    var incr, ilev;
    var r8iTox2 = new Array(216);
    for (i = 0; i<216; i++){
        r8iTox2[i] = new Array(9);
    }

    n = 216;    /* ** age categories boundaries */
    r = 0.0;
    //HACK ???? what is this supposed to mean
    ni = 0;
    ns = 0;
    ti = CurrentAge;
    ts = ProjectionAge;
 ...
 /*11/29/2007 SR: recode agemen for African American women*/
    ...
    if (ts <= t[ni])
    {
        ...
        abs[i - 1] = abs[i - 1] * rlan[ni - 1] * Math.exp(sumbb[i - 1]) / (rlan[ni - 1] * Math.exp(sumbb[i - 1]) + rmu[ni - 1]); /* breast cance */
    }
    else
    {
    ...
    abss = abs[i - 1] * 1000.0;
    //HACK CHECK THIS
    console.log("abss value")
    console.log(abss)
    abss_ = Math.ciel(abss) - 1;
    if (abss - abss_ >= .5)
    {
        //abss = d_int(abss) + 1.0 ;
        abss = abss + 1.0;
    ...
    abss = abss/10.0;   /* ** write the results to screen and output file */
    /*
...
document.write("Running Gail Function")
CalculateRisk(1,3,4,0,1,99,2,0,1,0,1,3);
